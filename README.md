...
Для начала необходимо оценить производительность языка программирования Go, на котором написан сервер. Ссылаясь на [методическое указание](https://github.com/init/highload/blob/main/highload_l11_hosting.md),
а также принимая тот факт, что go сопоставим с c++ по производительности, но проигрывает по памяти, произведём расчёты из предположения, 
что Go потребляет на 25% больше RAM. Тогда представим сводную таблицу производительности данного ЯП:

| Характер сервиса      | RPS  | RAM, Мб |
|-----------------------|------|---------|
| Лёгкое JSON API       | 5000 | 12.5    |
| Средняя бизнес-логика | 100  | 125     |
| Тяжёлая бизнес-логика | 10   | 125     |

### Нагрузка на сервисы и требуемые ресурсы

| Сервис      | Пиковый RPS | Характер сервиса      | CPU  | RAM, Гб |
|-------------|-------------|-----------------------|------|---------|
| Auth        | 1 млн.      | Лёгкая бизнес-логика  | 200  | 20      |
| Search      | 11600       | Тяжёлая бизнес-логика | 1160 | 145     |
| Profile     | 120 тыс.    | Средняя бизнес-логика | 1200 | 150     |
| Posts       | 200 тыс.    | Средняя бизнес-логика | 2000 | 250     |
| Feed        | 800 тыс.    | Лёгкая бизнес-логика  | 160  | 20      |
| API Gateway | 1 млн.      | Лёгкая бизнес-логика  | 200  | 4       |
|             |             |                       |      |         |
| Суммарный   | 3.2 млн.    |                       | 5000 | 600     |

Данные взяты из главы 2 "Расчёт нагрузки". Распределим типы запросов по сервисам:
1. Сервис авторизации:
   - Авторизация (380 RPS)
   - Регистрация (10)
   - Проверка авторизации (межсервисный запрос на каждый внешний запрос к другим сервисам - 1 млн. )  
Засчёт преобладания простых запросов на проверку сессии, общий характер сервиса - легкая бизнес-логика
2. Сервис поиска:
   - Поиск профиля (11600)
3. Сервис профиля:
   - Запрос профиля (11600 * 10) (При поиске профиля происходят межсервисные запросы, чтобы собрать карточки пользователей)
   - Подписка на профиль (5800)
4. Сервис постов:
   - Публикация постов (2100)
   - Лайки и комментарии (200 тыс.)
5. Сервис ленты:
   - Просмотр поста (800 тыс.)
6. API Gateway. Через данный компонент проходят все входящие запросы (1 млн.)
